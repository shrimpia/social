...
export const MfmNodeView = ({ node }: MfmNodeProp) => {
  switch (node.type) {
    ...
    case 'bold': {
      return <strong>{node.children.map((child, i) => <MfmNodeView key={i} node={child} />)}</strong>;
    }
    case 'italic': {
      return <em>{node.children.map((child, i) => <MfmNodeView key={i} node={child} />)}</em>;
    }
    case 'strike': {
      return <del>{node.children.map((child, i) => <MfmNodeView key={i} node={child} />)}</del>;
    }
    case 'link': {
      return (
        <a href={node.props.url} target="_blank" rel="noopener noreferrer nofollow">
          {node.children.map((child, i) => <MfmNodeView key={i} node={child} />)}
        </a>
      );
    }
    case 'mention': {
    ...
    case 'plain': {
      return node.children.map((child, i) => <MfmNodeView key={i} node={child} />);
    }
    case 'inlineCode': {
    ...
    case 'small': {
      return <small style={{ opacity: 0.7 }}>{node.children.map((child, i) => <MfmNodeView key={i} node={child} />)}</small>;
    }
    case 'center': {
      return <div style={{ textAlign: 'center' }}>{node.children.map((child, i) => <MfmNodeView key={i} node={child} />)}</div>;
    }
    case 'quote': {
      return <blockquote>{node.children.map((child, i) => <MfmNodeView key={i} node={child} />)}</blockquote>;
    }
    case 'search': {
